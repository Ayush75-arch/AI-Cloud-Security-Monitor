...
async def validate_terraform_upload(file: UploadFile, client_ip: str = "unknown") -> bytes:
    """
    ...
    if DANGEROUS_FILENAME_PATTERN.search(filename):
        log_upload_rejected(ip=client_ip, filename=filename, reason="dangerous filename pattern")
        raise HTTPException(status_code=400, detail="Invalid filename")

    # 2. Extension whitelist
    suffix = Path(filename).suffix.lower()
    if suffix not in ALLOWED_EXTENSIONS:
        log_upload_rejected(ip=client_ip, filename=filename,
    ...
    if b"\x00" in content[:512]:
        log_upload_rejected(ip=client_ip, filename=filename, reason="binary content detected")
