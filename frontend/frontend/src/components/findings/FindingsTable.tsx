...
export function FindingsTable({ findings, loading, onSuppressed }: Props) {
  ...
  // Fix 16: navigate to /chat with finding context pre-loaded
  ...
  return (
    <div className="flex h-full">
            ...
            {sorted.map((f, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 flex items-center gap-2">
                  {/* Fix 16: Ask AI button */}
                  <button
                    onClick={(e) => { e.stopPropagation(); handleAskAI(f) }}
                    className="font-mono text-2xs uppercase tracking-widest text-text-muted hover:text-accent-green px-2 py-1 border border-transparent hover:border-bg-border transition-colors"
      ...
      {/* Detail panel */}
      {selected && <FindingDetail finding={selected} onClose={() => setSelected(null)} onAskAI={handleAskAI} />}
    </div>
  )
}
    ...
    <div className="w-1/2 overflow-auto p-5 space-y-5 animate-slide-up">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {/* Fix 16: Ask AI from detail panel */}
          <button
            onClick={() => onAskAI(f)}
            className="font-mono text-2xs uppercase tracking-widest text-accent-green border border-accent-muted px-3 py-1 hover:bg-accent-muted transition-colors"
          >
            ◈ Ask AI
          </button>
